...
class DefiDataSeeder extends Seeder
{
    public function run()
    {
        // Create sample crowdfunding campaigns without property references
        $campaigns = [
            [
                'property_id' => null, // No property reference for now
        ...
        try {
            ...
            // If that fails, try without the property_id constraint
            $campaignsWithoutProperty = [];
            foreach ($campaigns as $campaign) {
                $campaignWithoutProperty = $campaign;
                unset($campaignWithoutProperty['property_id']);
                $campaignsWithoutProperty[] = $campaignWithoutProperty;
            }
            DB::table('crowdfunding_campaigns')->insert($campaignsWithoutProperty);
        }
        ...
        if ($campaigns->count() > 0 && $users->count() > 0) {
            $investments = [
                ...
                [
                    'campaign_id' => $campaigns[1]->id ?? $campaigns[0]->id,
        ...
        $loans = DB::table('defi_loans')->limit(2)->get();
        $repayments = [];
        ...
        if ($loans->count() > 0) {
            ...
            // Add some repayments for loan 2 if it exists
            if ($loans->count() > 1) {
                for ($i = 1; $i <= 4; $i++) {
                    $repayments[] = [
                        'loan_id' => $loans[1]->id,
        ...
        $assessments = [
            [
                'property_id' => 1, // Use a dummy property ID
            ...
            [
                'property_id' => 2, // Use a dummy property ID
        ...
        try {
            ...
            // If that fails, just skip risk assessments for now
            echo "Could not insert risk assessments: " . $e->getMessage();
        }
    }
}
