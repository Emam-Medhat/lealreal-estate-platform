...
@section('title', 'إنشاء تقرير سوق جديد')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-indigo-700 rounded-lg shadow-lg p-6 mb-6 text-white">
            <div class="flex items-center">
                <a href="{{ route('reports.market.index') }}" class="text-white hover:text-purple-200 mr-4">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <form method="POST" action="{{ route('reports.market.store') }}">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select id="market_area" 
                        ...
                        <option value="الرياض" {{ old('market_area') == 'الرياض' ? 'selected' : '' }}>الرياض</option>
                        <option value="جدة" {{ old('market_area') == 'جدة' ? 'selected' : '' }}>جدة</option>
                        <option value="مكة المكرمة" {{ old('market_area') == 'مكة المكرمة' ? 'selected' : '' }}>مكة المكرمة</option>
                        <option value="المدينة المنورة" {{ old('market_area') == 'المدينة المنورة' ? 'selected' : '' }}>المدينة المنورة</option>
                        <option value="الدمام" {{ old('market_area') == 'الدمام' ? 'selected' : '' }}>الدمام</option>
                        <option value="الخبر" {{ old('market_area') == 'الخبر' ? 'selected' : '' }}>الخبر</option>
                        <option value="الطائف" {{ old('market_area') == 'الطائف' ? 'selected' : '' }}>الطائف</option>
                        <option value="تبوك" {{ old('market_area') == 'تبوك' ? 'selected' : '' }}>تبوك</option>
                        <option value="أبها" {{ old('market_area') == 'أبها' ? 'selected' : '' }}>أبها</option>
                        <option value="نجران" {{ old('market_area') == 'نجران' ? 'selected' : '' }}>نجران</option>
                    </select>
                    @error('market_area')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <label class="flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                            <input type="radio" name="format" value="pdf" class="ml-3" {{ old('format', 'pdf') == 'pdf' ? 'checked' : '' }} required>
                            <div>
                        ...
                        <label class="flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                            <input type="radio" name="format" value="excel" class="ml-3" {{ old('format') == 'excel' ? 'checked' : '' }}>
                            <div>
                        ...
                        <label class="flex items-center p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                            <input type="radio" name="format" value="csv" class="ml-3" {{ old('format') == 'csv' ? 'checked' : '' }}>
                            <div>
                ...
                <div class="mb-6">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center">
                            <input type="checkbox" name="filters[property_type]" value="1" class="ml-3" {{ old('filters.property_type') ? 'checked' : '' }}>
                            <span>تحليل حسب نوع العقار</span>
                        </label>
                        
                        <label class="flex items-center">
                            <input type="checkbox" name="filters[price_range]" value="1" class="ml-3" {{ old('filters.price_range') ? 'checked' : '' }}>
                            <span>تحليل حسب نطاق السعر</span>
                        </label>
                        
                        <label class="flex items-center">
                            <input type="checkbox" name="filters[neighborhood]" value="1" class="ml-3" {{ old('filters.neighborhood') ? 'checked' : '' }}>
                            <span>تحليل حسب الأحياء</span>
                        </label>
                        
                        <label class="flex items-center">
                            <input type="checkbox" name="filters[competitor]" value="1" class="ml-3" {{ old('filters.competitor') ? 'checked' : '' }}>
                ...
                <div class="flex items-center justify-between">
                    <a href="{{ route('reports.market.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
                        إلغاء
                    </a>
