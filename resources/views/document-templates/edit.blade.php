...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">تعديل القالب</h1>
    ...
    <form method="POST" action="{{ route('document-templates.update', $template) }}" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ $template->name }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                </div>
                ...
                <div>
                    ...
                    <select name="category" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                        ...
                        <option value="contract" {{ $template->category == 'contract' ? 'selected' : '' }}>عقود</option>
                        <option value="legal" {{ $template->category == 'legal' ? 'selected' : '' }}>قانوني</option>
                        <option value="financial" {{ $template->category == 'financial' ? 'selected' : '' }}>مالي</option>
                        <option value="administrative" {{ $template->category == 'administrative' ? 'selected' : '' }}>إداري</option>
                        <option value="technical" {{ $template->category == 'technical' ? 'selected' : '' }}>فني</option>
                    </select>
                </div>
                
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">الوصف</label>
                    <textarea name="description" rows="3" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ $template->description }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div>
                ...
                <textarea name="content" rows="15" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>{{ $template->content }}</textarea>
                    ...
                    @foreach($template->variables as $index => $variable)
                        <div class="variable-item border border-gray-200 rounded-lg p-4 mb-4">
                            <div class="flex justify-between items-center mb-2">
                                <h3 class="font-medium text-gray-900">متغير {{ $index + 1 }}</h3>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input type="text" name="variables[{{ $index }}][name]" value="{{ $variable['name'] }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="مثال: client_name">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">الوصف</label>
                                    <input type="text" name="variables[{{ $index }}][description]" value="{{ $variable['description'] }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="مثال: اسم العميل">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">النوع</label>
                                    <select name="variables[{{ $index }}][type]" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                        ...
                                        <option value="currency" {{ $variable['type'] == 'currency' ? 'selected' : '' }}>عملة</option>
                                        <option value="email" {{ $variable['type'] == 'email' ? 'selected' : '' }}>بريد إلكتروني</option>
                                        <option value="phone" {{ $variable['type'] == 'phone' ? 'selected' : '' }}>هاتف</option>
                                    </select>
                                </div>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="direction" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="rtl" {{ $template->direction == 'rtl' ? 'selected' : '' }}>من اليمين إلى اليسار</option>
                        <option value="ltr" {{ $template->direction == 'ltr' ? 'selected' : '' }}>من اليسار إلى اليمين</option>
                    </select>
                </div>
                
                <div class="flex items-center">
                    <input type="checkbox" name="is_active" {{ $template->is_active ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    <label class="mr-2 text-sm text-gray-700">نشط</label>
                </div>
                
                <div class="flex items-center">
                    <input type="checkbox" name="requires_signature" {{ $template->requires_signature ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
        ...
        <div class="flex justify-end space-x-4 space-x-reverse">
            <a href="{{ route('document-templates.show', $template) }}" class="bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600">
                إلغاء
            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    let variableCount = {{ $template->variables ? count($template->variables) : 0 }};
