    ...
    <div class="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-800 rounded-2xl shadow-xl p-8 text-white">
        <div class="flex justify-between items-center">
            <div>
                <div class="flex items-center mb-3">
                    ...
                    <div>
                        ...
                        <p class="text-blue-100 mt-1">{{ $policy->warranty_number }} - {{ $policy->title }}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-4 mt-4">
                    <div class="bg-white bg-opacity-20 rounded-lg px-3 py-1">
                        <span class="text-sm text-blue-100">الحالة:</span>
                        <span class="text-sm font-semibold text-white">{{ $policy->status_label }}</span>
                    </div>
                    <div class="bg-white bg-opacity-20 rounded-lg px-3 py-1">
                        <span class="text-sm text-blue-100">النوع:</span>
                        <span class="text-sm font-semibold text-white">{{ $policy->warranty_type_label }}</span>
                    </div>
                </div>
            </div>
            <div class="flex space-x-reverse space-x-3">
                ...
                <a href="{{ route('warranties.policies.edit', $policy->id) }}" class="inline-flex items-center px-6 py-3 bg-white bg-opacity-20 backdrop-blur-sm border border-white border-opacity-30 rounded-xl text-sm font-medium text-white hover:bg-opacity-30 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white transition-all duration-300">
    ...
    <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-white">{{ number_format($policy->coverage_amount, 2) }}</p>
    ...
    <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-white">{{ number_format($policy->coverage_amount - $stats['total_claimed_amount'], 2) }}</p>
    ...
    <div class="bg-gradient-to-br from-amber-500 to-amber-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-white">{{ $stats['days_remaining'] }}</p>
    ...
    <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-white">{{ $stats['total_claims'] }}</p>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 space-y-4">
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                ...
                <span class="text-sm font-semibold text-gray-900">{{ $policy->warranty_number }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">العنوان</span>
                <span class="text-sm font-semibold text-gray-900">{{ $policy->title }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">نوع الضمان</span>
                <span class="text-sm font-semibold text-gray-900">{{ $policy->warranty_type_label }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 space-y-4">
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                ...
                <span class="text-sm font-semibold text-gray-900">{{ number_format($policy->coverage_amount, 2) }} ريال</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">مبلغ الخصم</span>
                <span class="text-sm font-semibold text-gray-900">{{ number_format($policy->deductible_amount, 2) }} ريال</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">المبلغ المدفوع</span>
                <span class="text-sm font-semibold text-gray-900">{{ number_format($stats['total_claimed_amount'], 2) }} ريال</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">المتبقي</span>
                <span class="text-sm font-semibold text-gray-900">{{ number_format($policy->coverage_amount - $stats['total_claimed_amount'], 2) }} ريال</span>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 space-y-4">
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                ...
                <span class="text-sm font-semibold text-gray-900">{{ $policy->start_date->format('Y-m-d') }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">تاريخ الانتهاء</span>
                <span class="text-sm font-semibold text-gray-900">{{ $policy->end_date->format('Y-m-d') }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">المدة</span>
                <span class="text-sm font-semibold text-gray-900">{{ $policy->duration_months }} شهر</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">الأيام المتبقية</span>
                <span class="text-sm font-semibold text-gray-900">{{ $stats['days_remaining'] }} يوم</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="p-6 space-y-4">
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                ...
                <span class="text-sm font-semibold text-gray-900">{{ $policy->property->title ?? 'N/A' }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">مقدم الخدمة</span>
                <span class="text-sm font-semibold text-gray-900">{{ $policy->serviceProvider->name ?? 'N/A' }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">الشخص المسؤول</span>
                <span class="text-sm font-semibold text-gray-900">{{ $policy->contact_person ?? 'N/A' }}</span>
            </div>
            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                <span class="text-sm font-medium text-gray-600">رقم الهاتف</span>
                <span class="text-sm font-semibold text-gray-900">{{ $policy->contact_phone ?? 'N/A' }}</span>
            </div>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="p-6">
            <p class="text-gray-700 leading-relaxed">{{ $policy->description ?? 'لا يوجد وصف' }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        ...
        <div class="p-6">
            <p class="text-gray-700 leading-relaxed">{{ $policy->coverage_details ?? 'لا توجد تفاصيل' }}</p>
        </div>
    </div>
</div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $claim->claim_date->format('Y-m-d') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ number_format($claim->amount, 2) }} ريال</td>
