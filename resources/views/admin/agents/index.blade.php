    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-lg shadow-lg p-6 mb-6 text-white">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('admin.agents.create') }}" class="bg-white bg-opacity-20 hover:bg-opacity-30 text-white px-6 py-3 rounded-lg transition-all duration-300 flex items-center">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900">{{ $agents->total() }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-green-600">{{ $agents->where('status', 'active')->count() }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-yellow-600">{{ $agents->where('status', 'pending')->count() }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-red-600">{{ $agents->where('status', 'inactive')->count() }}</p>
                    </div>
            ...
            <form method="GET" action="{{ route('admin.agents.index') }}" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="flex space-x-2">
                        ...
                        <a href="{{ route('admin.agents.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                                        ...
                                        <div class="flex items-center">
                                            <div class="h-10 w-10 flex-shrink-0">
                                                <div class="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 flex items-center justify-center text-white font-semibold">
                                                    {{ strtoupper(substr($agent->name ?? $agent->first_name ?? 'A', 0, 1)) }}
                                                </div>
                                            </div>
                                            <div class="mr-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $agent->name ?? ($agent->first_name . ' ' . $agent->last_name) }}</div>
                ...
                <div class="bg-gray-50 px-6 py-3 border-t border-gray-200">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div class="text-sm text-gray-700 mb-4 sm:mb-0">
                            عرض {{ $agents->firstItem() }} إلى {{ $agents->lastItem() }} من {{ $agents->total() }} وكيل
                        </div>
                        <div>
                            {{ $agents->links() }}
                        </div>
