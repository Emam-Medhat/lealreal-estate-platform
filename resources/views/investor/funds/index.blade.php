...
@section('title', 'لوحة تحكم الصناديق')
@section('page-title', 'لوحة تحكم الصناديق')
                ...
                <form method="GET" action="">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                        <div class="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="البحث في الصناديق..." class="px-3 py-2 border rounded-lg text-sm w-full md:w-64">
                            <select name="fund_type" class="px-3 py-2 border rounded-lg text-sm">
                                <option value="">جميع الأنواع</option>
                                <option value="real_estate" {{ request('fund_type') == 'real_estate' ? 'selected' : '' }}>العقارات</option>
                                <option value="technology" {{ request('fund_type') == 'technology' ? 'selected' : '' }}>التكنولوجيا</option>
                                <option value="renewable_energy" {{ request('fund_type') == 'renewable_energy' ? 'selected' : '' }}>الطاقة المتجددة</option>
                                <option value="ecommerce" {{ request('fund_type') == 'ecommerce' ? 'selected' : '' }}>التجارة الإلكترونية</option>
                                <option value="education" {{ request('fund_type') == 'education' ? 'selected' : '' }}>التعليم</option>
                            </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <!-- Actions -->
                        <div class="flex gap-2">
                            <a href="/investor/funds/{{ $fund->id }}" class="flex-1 bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600 transition-colors text-center">
