...
class UpdatePropertySustainabilityRequest extends FormRequest
{
    /**
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            // Check for significant improvements that might require documentation
            $propertySustainability = $this->route('property_sustainability');
            if ($propertySustainability) {
                $improvementThreshold = 10; // 10 points improvement
                
                if ($this->filled('eco_score') && 
                    $this->eco_score - $propertySustainability->eco_score > $improvementThreshold) {
                    $validator->errors()->add('eco_score', 'التحسن الكبير في الدرجة البيئية (> 10 نقاط) يتطلب وثائق داعمة');
                }

                if ($this->certification_status === 'certified' && 
                    $propertySustainability->certification_status !== 'certified') {
                    if ($this->eco_score < 70) {
                        $validator->errors()->add('certification_status', 'لا يمكن اعتماد العقار بدرجة بيئية أقل من 70');
                    }
                }
            }
        });
