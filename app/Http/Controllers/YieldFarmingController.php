...
class YieldFarmingController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('blockchain.yield.index', compact('pools'));
    }

    public function createLiquidityPool(Request $request)
    {
        $request->validate([
            ...
            'protocol' => 'required|string|in:uniswap,sushswap,curve,balancer',
    ...
    public function getPoolPerformanceStats(Request $request)
    {
        $poolId = $request->pool_id;
    ...
    public function getPoolLiquidityStats($pool)
    {
        return $this->buildPoolLiquidityStats($pool);
    }
    ...
    private function buildPoolLiquidityStats($pool)
    {
        return [
            'total_liquidity' => $pool->token0_amount + $pool->token1_amount,
    ...
    private function getVolumeTrend($pool)
    {
        ...
        $firstHalf = array_slice($dailyVolumes, 0, (int) ceil(count($dailyVolumes) / 2));
        $lastHalf = array_slice($dailyVolumes, (int) ceil(count($dailyVolumes) / 2));
    ...
    {
        return DefiLoan::where('created_at', '>=', $startDate)
            ->selectRaw('(amount * fee_percentage / 100) as total_fees')
            ->value('total_fees');
    }
    ...
    {
        return DefiLoan::where('created_at', '>=', $startDate)
            ->selectRaw('protocol, COUNT(*) as count, SUM(amount) as total_volume')
            ->groupBy('protocol')
    ...
    {
        return DefiLoan::where('created_at', '>=', $startDate)
            ->selectRaw('provider_address, COUNT(*) as count, SUM(amount) as total_liquidity')
            ->groupBy('provider_address')
    ...
    {
        return DefiLoan::where('created_at', '>=', $startDate)
            ->selectRaw('(SUM(amount) / SUM(amount)) as utilization')
            ->value('utilization');
    }
    ...
    private function exportPoolsToCsv($pools)
    {
        ...
        $callback = function() use ($pools) {
            ...
            fputcsv($file, [
                'ID', 'Name', 'Address', 'Token0', 'Token1', 'Protocol', 'Type', 'Total Supply', 'Staked Amount', 'Reward Rate', 'Fee %', 'Active', 'Created At'
            ]);
